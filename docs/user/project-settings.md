...
Automatic icons check `t3.json`, common favicon and app icon paths, and icon links in project HTML
files. If no image is found, web and desktop show a colored monogram derived from the saved project
name. The badge stays the same when the sidebar shows a repository label such as `owner/repo`.
