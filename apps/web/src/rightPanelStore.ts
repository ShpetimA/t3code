    ...
    (set, get) => ({
      ...
      removeThread: (ref) =>
        set((state) => {
          ...
          const hasRevisionState = threadKey in state.userActionRevisionByThreadKey;
          if (!hasPanelState && !hasMaximizedState && !hasRevisionState) return state;
          ...
          const { [threadKey]: _removedMaximize, ...remainingMaximizedContent } =
            state.maximizedContentByThreadKey;
          const { [threadKey]: _removedRevision, ...remainingUserActionRevisions } =
            state.userActionRevisionByThreadKey;
          return {
            byThreadKey: remainingPanels,
            maximizedContentByThreadKey: remainingMaximizedContent,
            userActionRevisionByThreadKey: remainingUserActionRevisions,
          };
        }),
    }),
    {
