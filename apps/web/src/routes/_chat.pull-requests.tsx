...
function PullRequestsRouteView() {
  ...
  const globalSelected = globalTabsEnabled && pullRequestsSupported ? linkedSelection : null;
  ...
  const globalPullRequestTabKey =
    globalSelected !== null
      ? globalTabKey({
          _tag: "PullRequest",
          environmentId: globalSelected.environmentId,
          projectId: globalSelected.projectId,
          repository: globalSelected.repository,
          number: globalSelected.number,
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      ...
      rightControl={{
        _tag: "Panel",
        available: rightPanelState.surfaces.length > 0,
  ...
  const columnProps = {
    ...
    rightPanelControl:
      ...
      globalTabsEnabled || !pullRequestsSupported || rightPanelState.isOpen ? null : (
        ...
        {globalSelected !== null ? (
          <PreviewPanelShell mode="embedded">
            <PullRequestDetailPanel
              key={`${globalSelected.environmentId}:${globalSelected.projectId}:${globalSelected.repository}#${globalSelected.number}`}
              environmentId={globalSelected.environmentId}
              reference={globalSelected}
          ...
          <>
            {!globalTabsEnabled && pullRequestsSupported ? openPanelControls : null}
