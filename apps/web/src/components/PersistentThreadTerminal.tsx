...
interface PersistentThreadTerminalDrawerProps {
  ...
  readonly launchContext: PersistentTerminalLaunchContext | null;
  readonly focusRequestId: number;
  readonly splitShortcutLabel: string | undefined;
  readonly splitVerticalShortcutLabel: string | undefined;
...
interface PersistentThreadTerminalPanelProps {
  readonly threadRef: ScopedThreadRef;
  readonly surface: Extract<RightPanelSurface, { kind: "terminal" }>;
  readonly launchContext: PersistentTerminalLaunchContext | null;
  readonly focusRequestId: number;
  readonly keybindings: ResolvedKeybindingsConfig;
  readonly onAddTerminalContext: (selection: TerminalContextSelection) => void;
  readonly onSplitTerminal: () => void;
  readonly onSplitTerminalVertical: () => void;
  readonly onNewTerminal: () => void;
  readonly onActiveTerminalChange: (terminalId: string) => void;
...
function useThreadTerminalResources(threadRef: ScopedThreadRef) {
  ...
  return {
    ...
    threadWorktreePath: serverThread?.worktreePath ?? draftThread?.worktreePath ?? null,
  };
}

/** Keeps a thread's drawer terminals mounted while presenting only the active thread's drawer. */
export const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  const { project, knownTerminalSessions, threadWorktreePath } =
    useThreadTerminalResources(threadRef);
  ...
  const effectiveWorktreePath =
    launchContext !== null ? launchContext.worktreePath : threadWorktreePath;
  const cwd = useMemo(
    () =>
      launchContext?.cwd ??
      (project
  ...
  return (
    <div className={visible ? "shrink-0" : "hidden"}>
...
/** Presents a persistent terminal group inside a thread's right panel surface. */
export const PersistentThreadTerminalPanel = memo(function PersistentThreadTerminalPanel({
  ...
  const { project, knownTerminalSessions, threadWorktreePath } =
    useThreadTerminalResources(threadRef);
  const terminalSummariesById = useMemo(
    () =>
      new Map(
  ...
  const activeSummary = terminalSummariesById.get(surface.activeTerminalId) ?? null;
  const worktreePath =
  ...
  const terminalLabelsById = useMemo(() => {
    ...
    for (const terminalId of surface.terminalIds) {
      labels.set(
        terminalId,
        resolveTerminalSessionLabel(terminalId, terminalSummariesById.get(terminalId) ?? null),
      );
    }
    return labels;
  }, [surface.terminalIds, terminalSummariesById]);
  const terminalLaunchLocationsById = useMemo(() => {
    ...
    >();
    for (const terminalId of surface.terminalIds) {
      const summary = terminalSummariesById.get(terminalId) ?? null;
