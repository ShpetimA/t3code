...
function PullRequestSurfaceIcon({
  ...
  readonly surface: Extract<RightPanelSurface, { kind: "pull-request" }>;
  readonly environmentId: EnvironmentId | null;
  readonly seed: PullRequestTabStatusSeed | undefined;
}) {
        ...
        <div className="flex h-full w-max min-w-full items-center gap-1">
          {props.threadTab ? (
            <Tooltip>
              ...
              />
              <TooltipPopup>{props.threadTab.title}</TooltipPopup>
            </Tooltip>
          ) : null}
...
export function RightPanelTabs(props: RightPanelTabsProps) {
  ...
  const browserProfiles = useBrowserDefaults().profiles;
  const threadActive = props.threadTab?.active === true;
  return (
    <PreviewPanelShell
      mode={props.mode}
    ...
    >
      <RightPanelTabBar
        {...tabBarProps}
        activeSurfaceId={threadActive ? null : props.activeSurfaceId}
      />
      <div
