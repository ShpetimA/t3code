...
export const ChatHeader = memo(function ChatHeader({
  ...
  const openThreadActionMenu = useCallback(
    ...
    [activeProjectCwd, activeThreadRef, changeRequest, isServerThread, openMenu, startRename],
  );
  ...
  const openMenuFromTitle = useCallback(
    (event: ReactMouseEvent<HTMLButtonElement>) => {
      ...
      const clickedChevron = clickedElement?.closest("[data-thread-title-chevron]") != null;
  ...
  const handleTitleDoubleClick = useCallback(
    (event: ReactMouseEvent) => {
      ...
      const clickedElement = event.target instanceof Element ? event.target : null;
      if (clickedElement?.closest("[data-thread-title-chevron]") != null) return;
      cancelPendingTitleMenu();
      closeMenu();
      startRename();
    },
  ...
  const handleHeaderContextMenu = useCallback(
    (event: ReactMouseEvent) => {
      ...
      if (target?.closest("[data-chat-header-actions]")) return;
      cancelPendingTitleMenu();
      event.preventDefault();
      openThreadActionMenu({ x: event.clientX, y: event.clientY });
    },
    [cancelPendingTitleMenu, isServerThread, openThreadActionMenu, renamingTitle],
  );
